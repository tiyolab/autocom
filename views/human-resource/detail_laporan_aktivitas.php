...
foreach ( $HrdAktivitas->findLaporanAktivitasId($id) as $key => $value) {
	$this->title = 'Detail Aktivitas Pegawai : '.$value['nama_lengkap'];
}
				...
				<div class="row">
					<div class="col-md-12">
						<table class="table">
							...
							<tbody>
								<?php $i=1; foreach ( $HrdAktivitas->findLaporanAktivitasDetail($id) as $key => $value) { ?>
									<tr>
										<td><?=$i?></td>
										<td><?=$value['nip']?></td>
						...
						<a href="<?= Yii::$app->urlManager->createUrl(['human-resource/laporan-aktivitas'])?>" class="btn btn-primary btn-sm">Back</a>	
					</div>
				</div>
			</div>
	</div>
