    ...
    <div class="widget widget-green">
        <div class="widget-title">
			...
			<div class="widget-content">
				<div class="row">
					<div class="col-md-12">
						<a href="<?= Yii::$app->urlManager->createUrl(['human-resource/create-jadwal'])?>" class="btn btn-success btn-sm"><i class="glyphicon glyphicon-plus"></i></a>
						<table class="table">
							<thead>
								<tr>
									<th>No</th>
							...
							<tbody>
								<?php $i=1; foreach ( $HrdJadwal->findJadwal() as $key => $value) { ?>
									<tr>
										<td><?=$i?></td>
										<td><?=$value['materi_tes']?></td>
										<td><?=DateToIndo($value['jadwal_tes'])?></td>
										<td><?php
											if($value['persetujuan_jadwal']==1){
												echo "Disetujui";
											}else{
												echo "Ditolak";
											}											
										?></td>
										<td>
											<a href="<?= Yii::$app->urlManager->createUrl(['human-resource/update-jadwal',"id"=>$value['id_jadwal_tes']])?>" class="btn btn-success btn-sm"><i class="glyphicon glyphicon-pencil"></i></a>
											
											<a href="<?= Yii::$app->urlManager->createUrl(['human-resource/delete-jadwal',"id"=>$value['id_jadwal_tes']])?>" class="btn btn-danger btn-sm"><i class="glyphicon glyphicon-trash"></i></a>
										</td>
									</tr>
								<?php $i++; } ?>
							</tbody>
