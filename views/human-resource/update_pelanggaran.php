...
$myData = HrdPelanggaran::find()->where(['id_pelanggaran'=>Yii::$app->request->get()['id']])->asArray()->one();
				...
				<div class="row">
					<div class="col-md-12">
						<div class="form-group">							
							...
							<?= Html::input("text", "pelanggaran", $myData['pelanggaran'], ["placeholder"=>"Pelanggaran", "required"=>"required", "class"=>"form-control"]) ?>
							</br><?= Html::label( "Tanggal Melanggar", ["class"=>"form-control"]) ?>						
							<?= Html::input("date", "tgl_melanggar", $myData['tgl_melanggar'], [ "required"=>"required", "class"=>"form-control"]) ?>
							</br><?= Html::label( "Tindakan", ["class"=>"form-control"]) ?>						
							<?= Html::input("text", "tindakan", $myData['tindakan'], ["placeholder"=>"Tindakan", "required"=>"required", "class"=>"form-control"]) ?>				
						</div>
						<div class="form-group">
