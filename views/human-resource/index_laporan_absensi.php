...
$this->title = 'Laporan Absensi Pegawai';
				...
				<div class="row">
					<div class="col-md-12">
						<table class="table">
							...
							<tbody>
								<?php $i=1; foreach ( $HrdAbsensi->findLaporanAbsensi() as $key => $value) { ?>
									<tr>
										<td><?=$i?></td>
										<td><?=$value['nip']?></td>
										<td><?=$value['nama_lengkap']?></td>
										<td><?=$value['nama_jabatan']?></td>
										<td><?=$value['jumlah_absensi']?></td>
										<td><?=DateToIndo( $value['tanggal'])?></td>
										<td>										
											<a href="<?= Yii::$app->urlManager->createUrl(['human-resource/detail-laporan-absensi',"id"=>$value['id_pegawai']])?>" class="btn btn-primary btn-sm"><i class="glyphicon glyphicon-search"></i></a>
